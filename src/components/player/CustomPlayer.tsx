...
export function CustomPlayer({ 
                ...
                if (data.fatal) {
                    switch (data.type) {
                        ...
                        case Hls.ErrorTypes.MEDIA_ERROR:
                            ...
                            break;
                        default:
                            // Attempt to reload the entire stream
                            setTimeout(() => {
                                hls?.loadSource(videoUrl);
                                hls?.attachMedia(video);
                            }, 500);
    ...
    const playerContent = (
        ...
        >
            ...
            {/* Tap/click video area: play ↔ pause (controls bar is above at z-70) */}
            <button
                type="button"
                aria-label={isPlaying ? 'إيقاف' : 'تشغيل'}
                className="absolute inset-0 z-40 cursor-pointer bg-transparent border-0 p-0 m-0"
                onClick={(e) => {
                    e.stopPropagation();
